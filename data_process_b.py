...
test_b=pd.read_csv('wechat_algo_data1/test_b.csv')
...
def creat_npz(data):
    ...
    for col in one_hot_cols+['userid','feedid']:
        ...
        if col == 'userid':
            ...
            df_feature = sparse.hstack((df_feature,arr))
            print(col,int(time.time()-s),'s')
    sparse.save_npz("data_process_b/one_hot_cols.npz",df_feature)
    ...
    for col in vec_cols:
        ...
        arr=[]
        print(f'{col} save...')
        sparse.save_npz(f"data_process_b/vec_cols_{col}.npz",df_feature)
        print(col,int(time.time()-s),'s')
        df_feature=pd.DataFrame()
    ...
    for col in vec_cols2:
        ...
        arr=[]
        print(col,int(time.time()-s),'s')
    sparse.save_npz("data_process_b/vec_cols2.npz",df_feature)

data=pd.concat([user_action[['userid','feedid']],test_b[['userid','feedid']]],ignore_index=True)
...
for y in ys:
    for npz_file in ['one_hot_cols','vec_cols2']:
        ...
        feature = sparse.hstack((feature,sparse.load_npz(f'data_process_b/{npz_file}.npz').tocsr()[:,col]))
        print(y,npz_file,feature.shape)
        sparse.save_npz(f'data_process_b/{npz_file}_{y}.npz',feature)
...
feed_embeddings_pca=pd.concat([user_action[['userid','feedid']],test_b[['userid','feedid']]],ignore_index=True)
...
feed_embeddings_pca.to_hdf('data_process_b/feed_embeddings_pca_b.h5',key='pca',mode='w')